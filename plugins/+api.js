	...
	try {
        ...
        const sendStickerFromUrl = async(to, url) => {
            ...
            if(mime.split("/")[0] === "audio"){
		        ...
		        await elaina.sendMessage(from, `Hmm maapya kak saya kick, dilarang share link di group ini 🌿`, text , {quoted: mek})
		        elaina.groupRemove(from, [kic]).catch((e)=>{reply(`Bot harus menjadi admin terlebih dahulu`)})
		        }
              ...
              // SIAPA AKU
                ...
                if (budy.toLowerCase() == jawaban) {
                    ...
                    reply2("Maaf jawaban kamu salah!")
			...
			if (tebakgambar.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
                ...
                if (budy.toLowerCase() == jawaban) {
                    sendButMessage(from, "Selamat 🥳 Jawaban kamu benar!", ` ${ownername}`, [{"buttonId": `.tebakgambar`,"buttonText": {"displayText": "Main Tebak Gambar Lagi"},"type": "RESPONSE"}], {quoted : mek})
                    delete tebakgambar[sender.split('@')[0]]
                    fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
                } else {
                    reply("Maaf jawaban kamu salah!")
                }
            }
...
if (!iselaina) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
				...
				break
case 'infobot':
if (!iselaina) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verikasi`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			reply('Green Tea Bot')
break
case 'owner':{
if (!iselaina) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verikasi`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
	...
	.then(async(result) => {
		for(let i of result.medias){
			if(i.url.includes('mp4')){
             ...
             break
case 'tiktok':
sendButLocation(from, 'Silahkan pilih media yang ingin kamu download', ' ' + ownername, thumbnail, [{buttonId: `.tiktokwm ${q}`, buttonText: {displayText: 'Dengan Watermark'}, type: 1},{buttonId: `.tiktoknowm ${q}`, buttonText:{displayText: 'Tanpa Watermark'}, type: 1},{buttonId: `.tiktokmusic ${q}`, buttonText:{displayText: 'Audio'}, type: 1}], {quoted: mek})					
             break
case 'randommeme':
...
await elaina.sendMessage(from, get_result, image, {quoted: mek, caption: 'Random Meme 🍵'})
break
case 'siapakahaku': case 'siapaaku':{
		if (isGame(sender, isPremium, isCreator, isOwner, gcounttuser, glimit)) return sendButMessage(from, lang.limitg(prefix), ` ${ownername}`, [{buttonId: 'limit', buttonText: {displayText: `Cek limit`, },type: 1,}]);
...
case 'memegenerator': case 'memegen':{
						...
						if (args.length < 1) return reply(`Kirim perintah *${prefix + command}* Green Tea Bot`)
