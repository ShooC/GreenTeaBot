...
export function before(m, { isBotAdmin }) {
    ...
    if (chat.antiBadword && isBadword) {
        user.warning += 1
        this.sendButton(m.chat, `*🌿ᴛᴏxɪᴄ ᴛᴇʀᴅᴇᴛᴇᴋꜱɪ !\nhttps://greenteabot.ga/ ᯤ*
あ Warning: ${user.warning} / 5 ┊

[⚠️] Jika warning mencapai 5 kamu akan *dibanned+kick*

Anda hanya bisa meminta Owner untuk membuka banned-nya !\n\n

“Barang siapa yang beriman kepada Allah dan Hari Akhir maka hendaklah dia berkata baik atau diam” (HR. al-Bukhari dan Muslim).`, wm, [['Cek Warn', '.cekwarn'], ['Creator Green Tea Bot', '.owner']], m)
        if (user.warning >= 5) {
            user.banned = true
            if (m.isGroup) {
                if (isBotAdmin) {
